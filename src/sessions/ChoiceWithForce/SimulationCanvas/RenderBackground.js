...
export const drawEmanations = (ctx, cx, cy, time, pState, maxR) => {
    for (let i = 0; i < 4; i++) {
        ...
        for (let j = 0; j < 4; j++) {
            if (pState.subVesselOpacities[i][j] * phaseMasterOpacity > 0.01) {
                const layerRadius = Math.max(0.1, breatheRadius - (j * 4 / pState.zoomLevel));
                ctx.beginPath();
        ...
        if (i === 3) {
            for (let j = 0; j < 5; j++) {
                if (pState.restrictionOpacities[j] > 0.01) {
                    ...
                    const restrictBreatheR = Math.max(0.1, r * restrictRadiusScale + Math.sin(time * 1.5 + i + j) * 1.5);
                    ctx.beginPath();
    ...
    for (let k = 0; k < 5; k++) {
        ...
        if (layer && layer.windowProgress > 0.01) {
            ctx.save();
            const outerR = phase4Radius * (1 - (k * 0.10));
            const innerR = Math.max(0.1, phase4Radius * (1 - ((k + 1) * 0.10)));

            const bOuter = Math.max(0.1, outerR + Math.sin(time * 1.5 + 3 + k) * 1.5);
            const bInner = Math.max(0.1, innerR + Math.sin(time * 1.5 + 3 + (k + 1)) * 1.5);
            ...
            if (layer.windowFillProgress > 0.01) {
                ctx.beginPath();
                ctx.arc(cx, cy, Math.max(0.1, outerR), 0, Math.PI * 2, false);
                ctx.arc(cx, cy, Math.max(0.1, innerR), 0, Math.PI * 2, true);
